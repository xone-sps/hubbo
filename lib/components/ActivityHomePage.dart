import 'dart:convert';
import 'dart:developer';
import 'package:flutter/material.dart';
import 'package:hubbo/widget/CircularProgress.dart';
import 'package:hubbo/widget/GridTilesCategory.dart';
import 'package:hubbo/models/ActivityModel.dart';
import 'package:hubbo/utils/Urls.dart';
import 'package:http/http.dart';

ActivityModel activityModel;

class ActivityHomePage extends StatefulWidget {
  String slug;
  bool isSubList;

  ActivityHomePage({Key key, this.slug, this.isSubList=false}) : super(key: key);
  @override
  _ActivithyHomePageState createState() => _ActivithyHomePageState();
}

class _ActivithyHomePageState extends State<ActivityHomePage> {
  @override
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: getCategoryList(widget.slug, widget.isSubList),
      builder: (context, AsyncSnapshot snapshot) {
        switch (snapshot.connectionState) {
          case ConnectionState.none:
          case ConnectionState.waiting:
            return CircularProgress();
          default:
            if (snapshot.hasError)
              return Text('Error: ${snapshot.error}');
            else
              return createListView(context, snapshot);
        }
      },
    );
  }
}

Widget createListView(BuildContext context, AsyncSnapshot snapshot) {
  ActivityModel values = snapshot.data;
  List<Data> results = values.data;
  return GridView.count(
    crossAxisCount: 2,
    padding: EdgeInsets.all(1.0),
    childAspectRatio: 8.0 / 9.0,
    children: List<Widget>.generate(results.length, (index) {
      return GridTile(
          child: GridTilesCategory(
              name: results[index].shopName,
              imageUrl: results[index].shopImage,slug:results[index].slug));
    }),
  );
}

Future<ActivityModel> getCategoryList(String slug, bool isSubList) async {
  if (isSubList) {
    activityModel = null;
  }
  if (activityModel == null) {
    Response response =
    await get(Urls.ROOT_URL + slug);
    int statusCode = response.statusCode;
    var body = json.decode(response.body);
    log('${body}');
    if (statusCode == 200) {
      activityModel = ActivityModel.fromJson(body);
//    brandModel = (body).map((i) =>BrandModel.fromJson(body)) ;
      return activityModel;
    }
  } else {
    return activityModel;
  }
}
//https://api.evaly.com.bd/core/public/category/shops/bags-luggage-966bc8aac/?page=1&limit=15